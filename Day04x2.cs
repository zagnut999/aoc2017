    ...
    public class Day04x2
    {
        ...
        */
        ...
        private bool TestPassword(string password)
        {
            ...
            {
                var count = words.Count(x => AreEqual(x, word));
        ...
        private static bool AreEqual(string thing1, string thing2)
        {
            if (thing1 == thing2)
                return true;
            ...
            {
                var thing1Char = thing1.ToCharArray().OrderBy(x => x).ToList();
                var thing2Char = thing2.ToCharArray().OrderBy(x => x).ToList();

                for (var i = 0; i < thing1Char.Count; i++)
                {
                    if (thing1Char[i] != thing2Char[i])
                        return false;
                }
                return true;
            }
