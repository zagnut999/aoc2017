...
Given that exactly one program is the wrong weight, what would its weight need to be to balance the entire tower?
        ...
        public void First()
        {
            ...
            var actual = FindMissingWeight(root);
        ...
        private int FindMissingWeight(Node node)
        {
            var weights = new Dictionary<int, List<Node>>();
            foreach(var child in node.Children)
            {
                if (weights.Keys.Contains(child.TotalWeight))
                {
                    weights[child.TotalWeight].Add(child);
                }
                else
                {
                    weights.Add(child.TotalWeight, new List<Node> {child});
                }
            }

            var incorrectNode = weights.FirstOrDefault(x => x.Value.Count == 1).Value?.FirstOrDefault();

            // The children are fine, it must be me
            if(incorrectNode == null)
                return 0;

            int missingWeight = 0;
            if (incorrectNode.Children.Count > 0)
                missingWeight = FindMissingWeight(incorrectNode);
                
            if (missingWeight != 0)
                return missingWeight;

            var childrenWeight = incorrectNode.Children.Sum(x => x.TotalWeight);
            var targetWeight = weights.First(x => x.Value.Count != 1).Key;

            var correctedWeight = targetWeight - childrenWeight;
            
            return correctedWeight;
        }

        private Node FindRoot(List<string> input)
        {
        ...
        private class Node
        {
            ...
            public int TotalWeight => Weight + Children.Select(x => x.TotalWeight).Sum();
